...
import { getActivityByCoordinatorDetails, getSchoolYearOptions } from "@/lib/legacy-reports-queries";
...
const BASE_PATH = "/reports/activity-by-coordinator-details";
...
export default async function ActivityByCoordinatorDetailsPage({
  searchParams,
}: {
  ...
  const [{ groups, schoolYear }, schoolYears] = await Promise.all([
    getActivityByCoordinatorDetails(filters),
    getSchoolYearOptions(),
  ]);

  return (
    <div className="flex flex-1 flex-col gap-5">
      <ReportToolbar title="Activity Detail by Coordinator" xlsxHref={xlsxHref(filters, schoolYear?.legacyId)} />
      ...
      <div className="print:block hidden text-[13px] font-bold">
        Activity Detail by Coordinator — {schoolYear?.label ?? ""}
          ...
          >
            <summary className="cursor-pointer px-5 py-3 text-[14px] font-extrabold" style={{ color: "var(--heading)" }}>
              ...
              <span className="font-normal" style={{ color: "var(--muted)" }}>
                — {group.schools.length} school{group.schools.length === 1 ? "" : "s"}, {group.total} total activity participation
              </span>
            </summary>
                ...
                <details key={school.schoolId} className="rounded-[10px] border ml-2 print:break-inside-avoid" style={{ borderColor: "var(--border)" }}>
                  ...
                  <div className="border-t" style={{ borderColor: "var(--border)" }}>
                    <div
                      ...
                      style={{ gridTemplateColumns: "1.7fr 1.3fr 70px", color: "var(--muted)" }}
                    >
                      <span>Activity</span>
                      <span>Item</span>
                      <span>Qty</span>
                    ...
                    {school.rows.map((row, i) => (
                      <div
                        ...
                        style={{ gridTemplateColumns: "1.7fr 1.3fr 70px" }}
                      >
                        <span style={{ color: "var(--text)" }}>
                          {formatDate(row.activityDate)} - ({row.activityLegacyId}) {row.activityDescription || "—"}
                        </span>
                        <span style={{ color: "var(--muted)" }}>{row.detailDescription}</span>
